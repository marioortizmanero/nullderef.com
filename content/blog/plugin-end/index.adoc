...
* Flamegraph
* `perf.data`: `perf report --no-children`: reduced `memmove` doesn't mean
  there's more move necessarily, just that something else got slower. Tip: take
  a screenshot and draw lines.
  TODO: look into what kernel.kallsyms is
...
image::/blog/plugin-end/rustc_crash.png[width=80%, align=center]
