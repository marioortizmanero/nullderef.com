...
.Variance comparison with the entire setup. Unexpectedly, the server ended up being worse in that regard for this run, but it was good enough to continue.
...
One of my main performance concerns with the first version of the PDK had to do
with https://nullderef.com/blog/plugin-impl/#_reaching_reprc_blockers[the issues
I had found when trying to convert external types to the C ABI]. I needed to
modify `Value` to an FFI-compatible type in order to use it in the plugin
system. However, it was also used throughout the entire codebase, and this
would've required a large amount of changes for just the first version of the
PDK. Thus, I created the copy `PdkValue` and converted to `Value` and back when
going through the FFI boundary.

I was able to prove its performance effect with some flamegraphs and call
stacks:
...
.Flamegraph with a single `Value`. The conversion calls add up to 5.2% now.
image::/blog/plugin-end/without_value_conv.png[]

Our program will now spend 4.2% less of its execution time converting between
`Value` types.

=== `abi_stable`

Another performance issue I found with `abi_stable` was its destructors.
