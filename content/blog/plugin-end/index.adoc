...
Welcome to this https://nullderef.com/series/rust-plugins/[series'] final
article! Here I'll showcase some clean-ups and optimizations I may or may not
have performed yet, so that our plugin system is closer to production. I will
also run benchmarks for some of these to ensure that the changes are actually
worth it.
...
relatively complex type named `Value`, which I needed to make FFI-safe for the
plugin system. However, it was used throughout the entire codebase, and
modifying it would've required a large amount of unrelated changes for just an
initial version. Thus, I created the copy `PdkValue`, and converted to `Value`
and back when going through the FFI boundary. `PdkValue` was only used for the
plugin system, making it quite easy to implement in contrast.
...
wrapper for the original external hash table implementation.
...
optimization I mentioned could also be worth removing now. But it's really hard
to make a decision just out of this set of benchmarks because of the complexity
of the change. 
...
need to analyze at the impact of using `hashbrown` without the plugin system,
and understand in detail _why this happens_. Taking a look at the flamegraphs,
for example, could help in figuring out the underlying reasoning.
...
* *Supporting more components* in the plugin system. We could continue to split
  up the executables into even smaller pieces, improving the overall flexibility
...
* *Security* is something we sacrificed when choosing dynamic loading as our
  base technology. We don't have a sandbox, which would be essential if a plugin
  repository were to exist.
* *Restarting or unloading plugins* at runtime. Unloading is especially
  complicated, considering `abi_stable` explicitly doesn't support it
  [REF_MISSING]. But restarting them could work by stopping and leaking existing
  ones, and then loading the same plugin with the same config. That would
  improve error resilience considerably, as we would be able to restart plugins
  upon failure (with limits, as leaking too much could crash everything).

== Conclusion

That was all! In summary, we initially slowed down our program by around 36%.
With a few iterative optimizations, we've reduced that number to around 30%.
It's a noticeable improvement, but there's still a long way to go until it's
good enough for production, I would say. There are so many ideas to try out, and
the ones I listed here are just a fraction.

I look forward to seeing how the plugin system continues to evolve with the
program!
