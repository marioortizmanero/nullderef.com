...
This is what was first implemented. It was quite simple and it worked. The
interface is similar to how {{< crate reqwest >}} works: there's a `blocking`
module in the crate with the exact same functionality as the root module, but
without `async` and `.await`. Instead of importing `rspotify::Client`, you do
`rspotify::blocking::Client`, and voilà! Your code is now blocking. And if we
don't want overhead for the async users we can just feature-gate it under the
...
the blocking code, which just calls `block_on` internally. This way you only
copy the _definitions_, but the implementation remains de-duplicated.
...
`is_sync` feature when compiling the crate. It works for functions, traits and
`impl` blocks. And you can specify custom async/sync implementations with the
`async_impl` and `sync_impl` procedural macros, respectively. It does its job
perfectly, and we've been using it for Rspotify for a while now with no problems
whatsoever.
...
flags in their `Cargo.toml`. For example, if `client-ureq` is configured, since
`ureq` is synchronous, it would enable `maybe_async/is_sync`. The HTTP trait
would have their `async`/`.await` removed, and the client would use it with that
interface.

This essentially means that this solution has none of the downsides I listed in
previous attempts:

* No code duplication at all
* No overhead neither at runtime nor at compile time. If the user wants a
  blocking client, they can configure `ureq`, which doesn't pull `tokio` and
  friends
