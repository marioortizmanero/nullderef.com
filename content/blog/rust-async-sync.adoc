...
* https://github.com/rust-lang/rfcs/pull/2962[This RFC for the Rust compiler]
  suggested adding the `oneof` configuration predicate to support exclusive
  features. This only makes it easier to have conflicting features for cases
  where there's no choice, but features should still be strictly additive.
* https://github.com/rust-lang/cargo/issues/8367
* https://github.com/rust-lang/cargo/issues/7924
...
comment], it's not something the Rust team doesn't want to support. But these
discussions are relatively recent, so there hasn't been much progress yet.

Later on
https://internals.rust-lang.org/t/pre-rfc-cargo-mutually-exclusive-features/13182/27[this
was discussed in the context of Cargo].
