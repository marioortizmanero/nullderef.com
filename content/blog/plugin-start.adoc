...
You can see an explanation in detail
https://github.com/WebAssembly/interface-types/blob/master/proposals/interface-types/Explainer.md[here].
And
https://github.com/WebAssembly/interface-types/blob/master/proposals/interface-types/Explainer.md#defining-language-neutral-interfaces-like-wasi-revisited[here's
an example] of how it might work for a function in C++. It's pretty neat.
...
* At the plugin level the `wasm_bindgen` crate seems to be ideal. It's a very
  simple procedural macro that can be added to the exported functions in the
  plugin in order to add support for Interface Types.
...
--
* With *`witx` files*. As
  https://github.com/WebAssembly/WASI/blob/main/docs/witx.md[this document
  explains], it's an experimental file format to express WASI APIs. It's
  a partial implementation of the Interface Types, but it means that the
  interfaces have to be written in said format, which looks like this:
+
--
[source, witx]
----
...
https://docs.rs/wiggle/[`wiggle`]. For the previous example, wiggle's macro will
generate a `Calculator` trait TODO

I learned most of this from https://radu-matei.com/blog/wasm-api-witx/[radu's
guide], please check it out for more information!
--
* The hacky-but-working way, via *pointers and a shared
  ...
  https://docs.wasmer.io/integrations/examples/memory-pointers[Pointers] can be
  used from either side to access the data.
+
--
...
so a procedural macro like the now outdated
https://github.com/FreeMasen/wasmer-plugin[`wasmer-plugin`] could simplify it.
For now, https://github.com/bincode-org/bincode[`bincode`] can be used for the
serialization steps manually.
