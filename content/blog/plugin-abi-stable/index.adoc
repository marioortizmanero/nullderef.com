...
* `Pin` in `EventPayload`
* `value_trait::StaticNode` in `tremor_value::Value`

Same thing applies to external types. Some of Tremor's dependencies that aren't
available in `abi_stable` either:

* `async_broadcast::Sender` in `SourceReporter`
* `value_trait::StaticNode` in `tremor_value::Value`
