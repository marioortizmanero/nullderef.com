...
* No `AtomicBool` for `QuiescenceBeacon`
* No `Sender` for `SourceReporter`
* No `Pin` for `EventPayload`
* `StableAbi` propagates to even `tremor-script`
...
continue, or is it better to perhaps write wrappers?

"Impossible":

* `Value::StaticNode`, since it's an external crate (`value-trait`)
////
