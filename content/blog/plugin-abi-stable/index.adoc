...
types_]. When a type implements `StableAbi` and has the
`#[sabi(kind(Prefix(....)))]` attribute, another type with the `_Prefix` suffix
is generated that can actually be passed through the FFI barrier. They are
needed because `abi_stable` doesn't just need {repr-c} structs, but also some
kind of versioning to avoid breakage in future upgrades.

In summary, when we declare our module as a prefix type, these are the types
that will be generated:

* `Module`: the type used to declare the module's data, from which
  `Module_Prefix` and `Module_Ref` are generated.
* `Module_Prefix`: prefix fields of `Module`
* `Module_Ref`: pointer to `Module_Prefix`

The first version of our module should have the attribute
`#[sabi(last_prefix_field)]` on its last field. We will be able to add more
fields to the module in that same version _after_ `last_prefix_field`. Moving it
required a backward-incompatible version bump.
...
Most of the loading functionality is already handled by `abi_stable`. The root
module we're exporting implements the `RootModule` trait, which includes
functions to load the plugin, such as
