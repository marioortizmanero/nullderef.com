...
unwind <<panic-kinds>>. This means that every single function we export in the
plugin should wrap its contents in
...
* A guarantee that even with `extern "C"`, panicking is not undefined behavior,
  it'll just abort. Setting `panic=abort` is sound as well.
