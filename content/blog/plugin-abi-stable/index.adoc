---
title: "Plugins in Rust: Using Dependencies to Reduce the Pain"
description: "TODO"
...
to write it ourselves. If all you need is something simple like this, consider
...
{repr-c}, `cglue` achieves FFI-safety by using _opaque types_.
...
but since it's very likely we're going to end up writing unsafe code, it's good
to know about it. That's exactly what Miri is used for: detecting undefined
...
having to resort to it, I'll try to add Miri to Tremor's workflow (Continuous
Integration, mainly).
...
complicated, since our interface would already be {repr-C}, which is the most
...
So let's start comparing `abi_stable` with the example I wrote in the previous
post using raw dynamic linking. I've created the `abi-stable-simple` experiment
...
`RSlice<T>`, `RArc<T>`, etc) implements this type, and it's used to make sure
...
this with regular Rust:

.How we'd do it with regular Rust
[source, rust]
----
trait State: Debug, PartialEq {}
...
``abi_stable``'s safer and more convenient alternatives. The most basic one
works as follows:

.How it works with Interface Types
[source, rust]
----
#[repr(C)]
#[derive(StableAbi)]
...
of the previous snippet of code, so it acts as the "`trait`". But all it does is
establish `Serialize`, `Deserialize`, `Debug` and `PartialEq` as its
supertraits.

=== sabi_trait

If we wanted to specify functions in the trait or supertraits other than the
predefined set that's supported, we can use
...
Note that the trait has to be object-safe, so we can't have `PartialEq` as a
supertrait anymore.

.How it works with `sabi_trait`
[source, rust]
----
#[repr(C)]
#[derive(StableAbi)]
