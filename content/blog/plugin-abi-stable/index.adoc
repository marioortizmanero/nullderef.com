...
Hopefully these performance degradations won't be as noticeable in the final
version of the PDK. We'll use `sabi_trait` only when loading the library instead
of for each call. And having a more complex use-case will probably avoid such
incredible optimizations in the native code. You can find the full statistical
reports in the `criterion-reports` directory of the
