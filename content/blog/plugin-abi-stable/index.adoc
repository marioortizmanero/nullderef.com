...
// TODO: this is all wrong lol

Note that the benchmarks still don't represent a real usage of Tremor; it's just
using the plugin I described in this post with the `min` function. But we _can_
analyze other things, such as the performance differences between `abi_stable`
and raw dynamic loading -- I doubt it's worth implementing the final version
with both methods in order to run some benchmarks. Even if the results aren't
too meaningful I already have some code I can use for more-reliable benchmarks
in the future.
...
- [[[panic-book,      8]]] https://doc.rust-lang.org/book/ch09-01-unrecoverable-errors-with-panic.html
- [[[panic-ffi,       8]]] https://doc.rust-lang.org/nomicon/ffi.html#ffi-and-panics
