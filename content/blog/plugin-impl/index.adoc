...
In order to solve these we can:

* Implement wrappers for these libraries, which is a very cumbersome task in
  my experience. And the performance gains from the optimizations that they
  provide become less clear, as we're introducing overheads with the wrappers.
* Implement these libraries from scratch with {repr-c}. Obviously the most time
  consuming, but with this we actually make sure that it's as performant as it
  gets. For example, writing a wrapper for `beef` wouldn't make that much sense,
  because a wrapper via opaque types would require a pointer indirection
...
removing code wasn't also tricky in some situations. If the optimizations were
only performed internally and transparently to the hash map functionality, then
it would most likely be the wisest choice. But after trying to remove them I
...
that's not FFI-safe, as we covered in previous articles. This is what {{< crate
abi_stable >}} does in its `external_types` module for crates like {{< crate
crossbeam >}} or {{< crate parking_log >}}.

As you may see with the already existing examples, however, implementing
wrappers is a very cumbersome task. Even after you're done you'll have to keep
them up to date, so this will increase your maintainance burden as well. {{<
crate halfbrown >}} and {{< crate beef >}} are somewhat complex libraries, so I
decided this wasn't the best choice either.

=== Re-implement with {repr-c} from scratch

Basically the same as implementing a wrapper, but on steroids. It might seem
like overkill, but as far as I know it's the only choice in some scenarios,
because with it we can make sure it's as performant as it can get. In
``Value``'s example, the problematic types are optimizations, so writing a
wrapper may have a performance hit and render them useless.
