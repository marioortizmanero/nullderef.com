...
implementation of a hash map instead of `std::collections::HashMap`. It's based
on {{< crate hashbrown >}}, which was, in fact, merged into the standard library
at some point.
...
possible to fix it properly, but it might be too much effort at the moment.
...
introduce at least a mandatory pointer indirection).
...
impl<'value> From<PdkValue<'value>> for Value<'value> {
    fn from(original: PdkValue<'value>) -> Self {
        match original {
            ...
            PdkValue::Object(m) => {
                let m = m
                    .into_inner() // No `Box` dereference move magic here :(
                    .into_iter()
...
Suprisingly easy to get working! Two problems, though:
...
* *Performance*: passing the value to the plugin and converting it back in there
  means we're iterating the data twice.
+
With experiments that I'll include in the next article, I found out that these
conversions make up 5 to 10% of Tremor's execution time. Which is less than I
expected, but still not good enough for production.
