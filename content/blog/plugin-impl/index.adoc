...
asynchronous programming heavily, which is the case of Tremor. Anyway, turns out
that using `async` isn't that complicated!

In the previous post I introduced {{< crate async_ffi >}}, which
...
The only problem that this brought me was 

For example, the connector context passed to the plugin holds the `Sender<T>`
part of a channel that can be used to indicate the runtime that the connection
has been lost. This is because it's very likely that this connection drop
