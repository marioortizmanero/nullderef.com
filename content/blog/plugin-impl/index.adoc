...
up changing it a thousand times anyway. Some of these type names may not exist
yet, or may not be meant to be used for FFI. But it'll serve us as a list of
things to work on.
...
If you're using {{< crate libloading >}} directly then you would also be
implementing it via a struct with function pointers, and you'd need to store
metadata about the plugin with constants. So in the end, it boils down to the
same thing; just with different amounts of boilerplate.
...
each of these types hold, and on...
...
most likely be external types, but things like async are a bit complicated to
deal with as well. In the case of Tremor, the main issue was the `Value` type.
It's used to represent a JSON-like payload; roughly defined as follows:
...
is a type from the {{< crate value_trait >}} library that may hold a list of
basic types: numbers, booleans, or null:
...
We could solve this by applying the very same procedure again (hence
_recursively_). In this case, it will work in because there aren't any other
non-trivial types in our enum. We can make it optional under a feature flag so
that it's applied only for those that actually need {repr-c} in the library.
...
Since it's an external library, we'll have to make a Pull Request and cross our
fingers that the changes get accepted. In my case, it was accepted, so that was
all I had to do:

[.text-center]
{{< gh pr "simd-lite/value-trait" 14 "TODO" "paragraph" >}}
