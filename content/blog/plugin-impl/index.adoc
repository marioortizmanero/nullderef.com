...
via a struct with function pointers. You'd also need to store metadata about the
plugin with constants yourself. But in the end, it boils down to the same thing;
you're specifying the functions that the plugins must export.
...
standard library that don't have an alternative in `abi_stable`. These will most
likely be external types, but things like async are a bit complicated to deal
with as well. In the case of Tremor, the main issue was the `Value` type. It's
used to represent a JSON-like payload; roughly defined as follows:
...
We could solve this by applying the same procedure as before, which will work in
this case because there aren't any other non-trivial types in our enum. We can
TODO it to a feature flag so that it's applied only for those that actually need
to use the library in an FFI context thanks to `cfg_attr`:
...
// TODO: mention and link properly
