...
otherwise it'll take forever with a complex enough codebase. Maybe all of this
will be obvious to you, or you may prefer taking a different approach. But it
...
Afterwards, I removed these `allow` statements and ran `cargo fix && cargo fmt`.
Both cargo commands are built-in, and will automatically take care of the
...
NOTE: {{< crate halfbrown >}} is based on {{< crate hashbrown >}}, which was, in fact,
...
Problem: Tremor actually used asynchronous channels, such as
...
If your use-case is simple enough, callbacks might be enough. I
...
both the runtime and the functionality in `common`. This is exactly what
happened to me with Tremor as well.
...
.The {work} interface for the plugin system
image::separation-temporary.png[align=center, width=30%]

== Getting closer to deployment

Preparing the release for the plugin system has always been a quite complicated
task. I've never wanted to introduce too many changes at once because otherwise
it will become very hard to review and publish. This is why I decided to make
some step-by-step plans for the PDK release:

. *Release the first version of the PDK*
. *Separate the runtime from the interface*
. *Actually make the plugins external*
