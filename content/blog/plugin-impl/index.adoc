...
Before anything else, I'd like to give out some miscellaneous advice regarding
refactoring with Rust. You'll be introducing a good amount of changes, so it's
good to do some brief planning beforehand. Otherwise, it will take forever if
your codebase is complex enough.

The most important thing is: Don't get lost in details, we just want it to work
first of all. Hacks are OK in the first implementation as long as it runs:

* Ugly code
* Non-performant code
** If stuck with lifetimes, just slap a `.clone()` in there
