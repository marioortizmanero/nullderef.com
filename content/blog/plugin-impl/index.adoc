...
pointer cast or a match. So here's when the "`just make it work`" approach is
useful: we'll just keep the FFI barrier minimal and add conversions as early as
possible. After we're done, we'll see if there are any performance issues, and
then work on them.
...
map instead of `std::collections::HashMap`. {{< crate halfbrown >}} is based on
{{< crate hashbrown >}}, which was actually merged into the standard library at
some point.

{{< crate halfbrown >}} also has some additional functionality over ``std``'s
implementation. Some of it is actually available on Nightly, but it basically
means that it's not present in {{< crate abi_stable >}}'s `RHashMap`.

// TODO: explain https://doc.rust-lang.org/std/collections/hash_map/struct.HashMap.html#method.raw_entry[`raw_entry`],
