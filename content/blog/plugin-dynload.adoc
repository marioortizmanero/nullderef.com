...
serialize them into a shared memory. It also has a few more restrictions: only
one exported function per file is allowed, and the "PDK" must handle indirect
access through the host if you want plugins to call each others (there is no
multiple imports/exports) (? TODO: rephrase).
