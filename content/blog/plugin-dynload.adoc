...
`stable_abi` crate. I suggest we do both. We'll most likely end up using the
latter because it should be easier and safer, but it's still a very good idea to
know how `stable_abi` works under the hood.
...
to use dynamic loading for a plugin system.

In the next article I'll cover the same points when using `abi_stable`. Although
in my opinion it's a very large crate and somewhat hard to learn, most of the
problems this post exposes are greatly simplified thanks to it.
...
- [[[fat-binaries,   11]]] https://stackoverflow.com/a/63346857/11488352
- [[[panic-ffi,      11]]] https://doc.rust-lang.org/nomicon/ffi.html#ffi-and-panics
