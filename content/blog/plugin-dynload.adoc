...
kept as simple as possible. These managers will receive connection requests from
the pipeline and then redirect or read from it.
...
In the source part, `build_events` first pre-processes the raw binary data and
then the codec is applied. For the sinks, the inverse process is followed: in
`serialize_for_stream`, the data is first decoded with the codec, and then it's
post-processed as raw binary data in `on_event`.
...
Versioning could also be based on semantic versioning, which establishes that
incompatible API changes happen only when the major (__x__.0.0) version is
bumped.
