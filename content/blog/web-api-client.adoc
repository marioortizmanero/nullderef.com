...
for the Spotify API, which group up the endpoints by categories. Their endpoint
...
suitable for your specific API client. The only way to do that would be to
...
* Declare the method with `#[doc(hidden)]` so that it doesn't appear in the
  documentation.
* Use private functions outside the client instead.
