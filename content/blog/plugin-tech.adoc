...
articles I found about plugins in Rust, this may be more helpful to the
community.
...
Other more involved methods include <<swift-abi>>:
...
doesn't seem to be any better than <<ffi>> overall.
...
https://github.com/Tyleo/sharedlib[`sharedlib`], but they don't seem _that_
different <<ffi-comparison>>. It seems to be a lower-level implementation for any kind of FFI that
requires `unsafe` for almost everything -- what I was expecting. Based on it
there's https://github.com/emoon/dynamic_reload[`dynamic_reload`], which is very
...
provides macros to make the library loading simpler, based on `libloading`.
...
that, the same way as with <<scripting-langs>>, it's not viable to create an
internal interface for Tremor through C. This leaves us with Rust-to-Rust as the
only option, which is the easiest, but still has important inconvenients:
...
breaking backwards compatibility]. Fortunately, it has a
...
* The one and only Amos wrote an extremely detailed related blog post on
  fasterthanlime
  https://fasterthanli.me/articles/so-you-want-to-live-reload-rust[here],
  specifically about live reloading Rust.
...
. [{rank-good}]*5/5* *Rust availability*
. [{rank-bad}]*1/5* *Safety*
. [{rank-bad}]*0/5* *Backwards compatibility* (may be around [{rank-meh}]*3/5*
  if using `abi_stable`)
. [{rank-good}]*5/5* *Ease of porting existing implementations*
...
serialization. So it's basically the <<pipes>> approach.
...
binary that acts as a standalone program, where the parameters and communication
takes place via stdin and stdout, serializing and deserializing with
https://serde.rs/[serde]. It includes a few of them by default, like the status
bar, or a file manager. Very neat architecture!
...
* https://bevyengine.org/[`bevy`]: a very promising game engine whose features
  are implemented as plugins. Most times, these are loaded at compile-time, but
  the
