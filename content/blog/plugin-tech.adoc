...
. Awful safety: lots of `unsafe` usage is required with plenty of caveats
  ...
  in the interface between Operating Systems <<libloading-cross-platform>>,
...
. [{rank-good}]*5/5* *Rust availability*
. [{rank-bad}]*1/5* *Safety*
. [{rank-good}]*5/5* *Backwards compatibility* (may be [{rank-bad}]*0/5* if not
  serializing/using `abi_stable`)
. [{rank-good}]*5/5* *Ease of porting existing implementations*
...
- [[[ffi-comparison,16]]] https://docs.rs/dlopen/0.1.8/dlopen/#compare-with-other-libraries
- [[[ffi-dlopen-safety,17]]] https://docs.rs/dlopen/0.1.8/dlopen/#safety
- [[[swift-abi, 22]]] https://gankra.github.io/blah/swift-abi/
