...
one for what we actually need in a Plugin Development Kit. A Foreign Function
Interface can allow us to directly access resources in separately compiled
objects, also after the linking phase with dynamic loading.
...
to create an internal interface for Tremor through C. It's definitely not worth
it to allow plugins to be created outside of Rust.

==== Rust

Using Rust FFI is the easiest, but it still has a couple of inconvenients:
`unsafe` usage is required, and binary compatibility is not good.

As a way to improve the binary compatibility aspect, I found the
