...
serialize and deserialize the data with a flexible protocol. If something like
JSON is used to communicate between the core and its plugins, adding a field to
a message won't break anything, and removing one can be done via a deprecation
process. Unfortunately, this introduces some overhead the application may not be
interested in.

As to breakage due to compiler version mismatches, there are a couple ways this
can be fixed that I'll talk about in depth later on. Some technologies other
...
https://nullderef.com/blog/gsoc-proposal/#_defining_an_interface[But I already
talked about this in the past], so I won't get into details.
